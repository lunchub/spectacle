class MailDeliveriesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @mail_delivery.save
        ...
        format.json { render action: 'show', status: :created, location: @mail_delivery }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to mail_deliveries_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def mail_delivery_params
      params.require(:mail_delivery).permit(:name)
    end
end
